...
위궤양(胃潰瘍, gastric ulcer)은 위점막이 일부 손실되어 점막근층 이상에까지 도달하는 병변을 말하며, 소화성 궤양(消化性潰瘍, peptic ulcer disease)은 위궤양·십이지장궤양·문합부 궤양을 포괄하는 상부위장관 궤양 질환이다. KCD-8 기준 **K25~K28(소화성 궤양, Peptic ulcer)** 에 해당하며, 위궤양(K25)·십이지장궤양(K26)·부위 불명 소화성 궤양(K27)·위공장궤양(K28) 등으로 세분된다[교과서적 근거]. 한의학에서는 위궤양을 위완통(胃脘痛)·조잡(嘈雜)·비만(痞滿)의 범주에 포괄하며, 주로 비위허약(脾胃虛弱)·비위습열(脾胃濕熱)·간위불화(肝胃不和)·기체혈어(氣滯血瘀)·위음부족(胃陰不足) 등으로 변증(辨證)하여 한약·침(鍼)·혈위도포(穴位貼敷) 등 다양한 중재를 적용한다[^25][^27][^93]. Helicobacter pylori(H. pylori) 감염·비스테로이드성 소염진통제(NSAIDs) 사용이 주요 원인이며, 한의 중재는 제균 보조·점막 보호·재발 방지 측면에서 임상적 중요성이 강조된다[^65][^68][^78]. 본 문서는 근거기반 한의학 관점에서 위궤양·십이지장궤양에 대한 한의 중재의 임상 근거를 KCD 질환군별로 정리한다.
...
- **문합부 궤양 (K28)**: 위절제술 등 수술 후 문합 부위에 발생하는 궤양이다[교과서적 근거].
- **출혈·천공성 궤양**: 소화성 궤양의 대표적 합병증으로, 대황(大黃) 주정 추출물이 위·십이지장궤양 출혈에서 지혈 효과를 보고한 초기 임상 연구가 있다(임상시험, 312명)[^9].
...
한의학에서는 위궤양을 위(胃)의 수납(受納)·부숙(腐熟) 기능 실조와 비(脾)의 운화(運化) 기능 실조로 본다[교과서적 근거]. 식상(食傷)·감사(感邪)·우울(憂鬱)·비위소약(脾胃素弱) 등으로 비위(脾胃)의 승강(升降)이 실조되면 위기(胃氣)가 막혀 위완통·포만·소화불량이 발생한다[교과서적 근거]. 병기 전변(轉變)은 다음과 같은 경로를 따른다.

- **비위허약(脾胃虛弱)→비위허한(脾胃虛寒)**: 오랜 병으로 중양(中陽)이 손상되면 위완은통(胃脘隱痛)이 만성화되고 온중건비(溫中健脾)가 필요해진다[교과서적 근거].
- **간기울결(肝氣鬱結)→간위불화(肝胃不和)**: 정서 스트레스가 간의 소설(疏泄)을 저해해 위기(胃氣)를 범(犯)하면 창통(脹痛)·구역이 나타난다[교과서적 근거].
- **비위습열(脾胃濕熱)**: 습열이 중초(中焦)에 울결되면 구고(口苦)·구취·작열통이 나타나며, H. pylori 감염과 밀접한 관련을 보인다[^24][^59].
- **기체혈어(氣滯血瘀)**: 오랜 기체(氣滯)가 혈어(血瘀)로 이행되면 고정된 자통(刺痛)이 나타나고, COX-2와 H. pylori의 상호작용이 이러한 만성 비수렴성 염증·어혈 형성 기전과 관련됨을 시사하는 관찰연구가 있다[^53].
- **위음부족(胃陰不足)**: 만성 소모로 위음이 손상되면 조잡(嘈雜)·구건(口乾)이 나타난다[교과서적 근거].
...
### 3. 한의 중재의 작용 경로 — 전통 병기와 현대적 상관물

| 전통 병기 | 현대적 상관물(인간 관찰 자료) | 근거 |
|---|---|---|
| 비위허약·비기허 | 위점막 EGFR·EGF·VEGF·PCNA 발현 저하 및 재생능 저하 | [^22][^106] |
...
| 변증 | 주증 | 설맥 | 치법 | 대표방 |
|---|---|---|---|---|
| 비위허약(脾胃虛弱)·비기허(脾氣虛) | 위완은통(隱痛), 식후 포만, 식욕부진, 피로 | 설담(舌淡)·태박백(苔薄白), 맥세약(脈細弱) | 건비익기(健脾益氣)·거습(祛濕) | 보중익기탕(補中益氣湯)·사군자탕(四君子湯)·육군자탕(六君子湯) |
| 비위허한(脾胃虛寒) | 위완냉통, 온희(溫喜), 청량한 음식 기피 | 설담태백(舌淡苔白), 맥침지(脈沈遲) | 온중건비(溫中健脾) | 소건중탕(小建中湯)·황기건중탕(黃芪建中湯) |
| 비위습열(脾胃濕熱) | 구고(口苦)·구취·위완작열동통, 설사 | 설홍태황니(舌紅苔黃膩), 맥활삭(脈滑數) | 청열화습(淸熱化濕) | 반하사심탕(半夏瀉心湯)·삼황사심탕 계열 |
| 간위불화(肝胃不和)·간기범위(肝氣犯胃) | 정서 스트레스로 악화되는 창만·통증·트림 | 설홍태박(舌紅苔薄), 맥현(脈弦) | 소간화위(疏肝和胃) | 시호소간산(柴胡疏肝散)·사역산(四逆散) 가미 |
| 기체혈어(氣滯血瘀) | 고정된 자통(刺痛), 야간 악화, 흑변 병력 | 설암유어반(舌暗有瘀斑), 맥삽(脈澁) | 활혈화어(活血化瘀) | 실소산(失笑散) 가미·단삼음(丹蔘飮) |
| 위음부족(胃陰不足) | 조잡·구건인조(口乾咽燥), 공복 시 악화 | 설홍소태(舌紅少苔), 맥세삭(脈細數) | 양음익위(養陰益胃) | 익위탕(益胃湯)·사삼맥동탕(沙蔘麥冬湯) 가미 |

이 표는 임상 틀이지 동일 근거수준의 권고가 아니며, 변증은 개별 환자의 설맥·병력을 종합해 판단해야 한다.

### 2. 변증 감별표

| 감별점 | 비위허약형 | 비위습열형 | 간위불화형 | 기체혈어형 |
|---|---|---|---|---|
...
1. 상복부 통증·소화불량 등 주증상 확인 → 내시경으로 궤양 위치·크기·심도 확인[교과서적 근거]
2. H. pylori 감염 여부 확인(요소호기검사·조직검사·혈청검사)[^65][^78]
3. 위암·기질적 질환 배제(생검)[교과서적 근거]
4. NSAIDs·항혈소판제·항응고제 복용력 확인[교과서적 근거]
5. 출혈·천공·협착 등 red flag 확인[^9][^118]
6. 한의학적 변증(비위허약·비위습열·간위불화·기체혈어·위음부족) 시행

### 4. 반드시 배제해야 할 기질적 질환

- 위암·림프종 등 악성 종양(생검 필수)[교과서적 근거]
- 위식도역류질환(GERD)·기능성 소화불량[교과서적 근거]
- 심근허혈(협심증·심근경색): 상복부 통증으로 나타날 수 있어 고위험 환자에서 심장 원인을 먼저 배제[교과서적 근거]
- 담석증·췌장염 등 다른 상복부 통증 원인[교과서적 근거]
- 궤양성 출혈(흑색변·빈혈)·천공(급성 복통·복막염)·유문협착(연하곤란·구토) 등 합병증[^9][^118]

## 제4편 치료(치법·방약·침구)

### 1. 치법 위계

한의학적 치법은 변증에 따라 건비익기(健脾益氣)·온중건비(溫中健脾)·청열화습(淸熱化濕)·소간화위(疏肝和胃)·활혈화어(活血化瘀)·양음익위(養陰益胃)의 여섯 갈래로 크게 나뉘며, H. pylori 양성인 경우 제균 요법을 기본으로 하고 변증에 따른 한약·침을 보조적으로 병용하는 것이 원칙이다[^65][^78][^87]. **변증 없는 관행적 처방은 근거에 부합하지 않는다.**
...
- **온위서(溫胃舒)/양위서(養胃舒)**: H. pylori 양성 만성 위염·소화성궤양 환자에서 다기관 임상 근거를 확보했다(임상시험, 642명)[^27].
...
- **대황(大黃, Rhei Radix et Rhizoma)**: 주정 추출물이 위·십이지장궤양 출혈에서 지혈 효과를 보고한 초기 임상 연구가 있다(임상시험, 312명)[^9].
- **황련(黃連, Coptidis Rhizoma)/베르베린**: H. pylori 제균 보조 효과가 다수 메타분석·임상시험으로 뒷받침된다[^41][^49][^75].
- **감초(甘草, Glycyrrhizae Radix)**: 위장 염증 조절·점막 항상성 유지에 기여하는 생리활성 성분을 함유한다(문헌 고찰)[^120].
- **사인(砂仁, Amomi Fructus)**: 휘발유 성분이 H. pylori 감염 위염 환자의 혈소판활성인자 등 염증 매개체 발현에 영향을 미친다(임상시험, 80명)[^51].
- **오매(烏梅, Prunus mume)**: H. pylori 관련 만성 위염 개선에 효과를 보인다는 관찰연구·임상시험이 있다[^56][^57].
...
- **취혈 원리**: 오경상합(五經相合) 이론에 기반한 취혈이 소화성 궤양의 증상 개선·궤양 치유에서 양방 표준 약물과 유사한 효과를 보이나 H. pylori 음전율은 양방보다 낮아 제균 우선 시 약물 병행이 필요하다(임상시험, 57명)[^8].
- **중완(中脘, CV12)**: 다기관 무작위 대조시험에서 소화성 궤양 치료에 활용된 대표 혈위이다(임상시험, 276명)[^7].
- **십이지장궤양 침 치료**: 3주 침 치료와 생활 습관 개선을 병행했을 때 76%의 완전 회복률을 보고한 연구가 있다(임상시험, 21명)[^32].
- **자오유주(子午流注) 시간 침법**: 위산분비·가스트린·PGE1을 조절하여 소화성궤양 치료에 활용될 수 있음을 시사한다(임상시험)[^2].
- **초단파(UHF) 요법**: 십이지장궤양 치료에 보조적으로 활용된 초기 임상 연구가 있다(임상시험, 95명)[^31].
- **혈위 배혈표**: 중완(CV12)·족삼리(足三里, ST36)·위수(胃兪, BL21)·양릉천(陽陵泉, GB34)·내관(內關, PC6)이 위완통·오심 완화에 상용된다[교과서적 근거].
- **침+한약 병용**: 침과 중약 병용은 중약 단독보다 임상적 유효율·H. pylori 제균율이 유의하게 높고 재발률과 가스트린 수치가 유의하게 낮아 더 효과적이다(메타분석, 1,632명)[^93].
- **혈위도포(穴位貼敷)**: 혈위 약물 패치(AHP)의 유효성·안전성을 평가하는 체계적 고찰 프로토콜이 진행되어, 기존 치료의 부작용 감소·재발률 하락을 위한 보완 치료로의 가능성을 제시한다[^119].
...
#### ① 코드
K25(위궤양) — 급성/만성, 출혈 동반 여부에 따라 K25.0~K25.9로 세분[교과서적 근거].

#### ② 병인병기
비위허약·비위습열·간위불화·기체혈어·위음부족의 다양한 변증이 관여하며, 위완통·조잡·비만의 병기가 중심이다[교과서적 근거].
...
#### ⑥ 관리
H. pylori 양성이면 표준 제균 요법과 함께 청열·건비·화위 한약 및 중성약을 병용하여 제균율을 높이고 재발을 관리한다[^41][^65][^78]. NSAIDs·알코올·흡연·자극성 식이·과식 등 유발 요인을 줄이고 정서 스트레스·수면을 관리한다[^19][^32].

### 2. 십이지장궤양군 (KCD-8: K26)

#### ① 코드
K26(십이지장궤양) — 급성/만성, 출혈·천공 동반 여부에 따라 세분[교과서적 근거].

#### ② 병인병기
공복 통증·야간통이 특징적이며, 한열변증(寒熱辨證) 상태가 위점막 염증 지표와 관련된다[^37]. 비위허한·비위습열이 상대적으로 흔한 변증 유형이다[교과서적 근거].
...
### 3. H. pylori 관련 위염·전암성병변군 (KCD-8: K29.4~K29.5, B98.0)

#### ① 코드
K29.4(만성 위축성 위염)·K29.5(만성 위염, 상세불명)·B98.0(H. pylori를 원인으로 분류된 질환의 원인균으로서)[교과서적 근거]. 위궤양과 병태생리적 연속선상에 있어 함께 다룬다.
...
#### ⑥ 관리
제균 성공 여부를 요소호기검사 등으로 재확인하고, 위축성 위염·장상피화생 환자는 정기적 내시경 추적(위암 위험 관리)이 필요하다[교과서적 근거]. 육군자탕 등 비기허형 처방이 삶의 질 개선에 참고된다[^122].

### 4. NSAIDs·스트레스 유발 궤양군 (KCD-8: K25.-·T39.-, 스트레스 관련 급성 위점막 병변)

#### ① 코드
K25.-(약물 유발 위궤양 포함)·T39.3(NSAIDs 중독 관련), 외상 후 급성 스트레스성 위점막 병변은 별도 코드로 분류될 수 있다[교과서적 근거].

#### ② 병인병기
NSAIDs로 인한 위기(胃氣) 손상, 중증 외상·화상 등 스트레스로 인한 기혈 실조가 병기의 핵심이다[교과서적 근거].

#### ③ 병태생리
COX-1 억제에 따른 프로스타글란딘 감소, 중증 스트레스 상태에서의 미주신경-교감신경 불균형과 점막 혈류 저하가 관여한다[교과서적 근거].

#### ④ 치료
병용 침 치료가 외상성 뇌손상 후 스트레스성 위점막 손상 개선에 기여했다는 임상시험 근거가 있다(임상시험, 90명)[^103]. 이는 응급·중환자 영역에서 침 치료가 보조적으로 고려될 수 있음을 시사하나, 근거 수준이 제한적이므로 표준 스트레스궤양 예방요법(PPI 등)을 대체할 수 없다.

#### ⑤ 예후
조기 중재 시 점막 손상 회복 가능성이 높으나, 원발 질환(외상·중증질환)의 중증도에 따라 예후가 좌우된다[교과서적 근거].

#### ⑥ 관리
NSAIDs 장기 복용이 필요한 환자는 위점막 보호제 병용과 함께 건비화위(健脾和胃) 한약을 보조적으로 고려할 수 있으나, 근거가 제한적이므로 반드시 변증을 전제로 한다[^103].

### 5. 출혈·천공 합병증군 (KCD-8: K25.0/K25.4·K92.2)

#### ① 코드
K25.0(급성 위궤양, 출혈 동반)·K25.4(만성 또는 상세불명 위궤양, 출혈 동반)·K92.2(상세불명의 위장관 출혈)[교과서적 근거].

#### ② 병인병기
기체혈어(氣滯血瘀)가 만성화되어 낙맥(絡脈)이 손상되면 토혈(吐血)·흑변(黑便)이 발생한다[교과서적 근거].

#### ③ 병태생리
궤양 기저부 혈관 침식에 의한 출혈, 장막 전층 관통에 의한 천공이 주요 기전이다[교과서적 근거]. 대황 등 지혈 효과가 있는 본초가 활용된 초기 임상 연구가 있다(임상시험, 312명)[^9].

#### ④ 치료
급성 출혈·천공은 응급 내시경 지혈술·수술 등 양방 처치가 최우선이며, 한의 중재는 안정화 이후 보조적 회복 관리에 국한된다[교과서적 근거][^9]. 체질 분류가 합병증 발생 위험 예측에 참고된다[^118].

#### ⑤ 예후
출혈·천공은 사망률과 관련된 중증 합병증으로, 조기 진단·처치가 예후를 좌우한다[교과서적 근거].

#### ⑥ 관리
출혈·천공 병력이 있는 환자는 재발 방지를 위한 H. pylori 제균·NSAIDs 회피·정기 추적이 필수적이며, 한의 중재는 회복기 소화기능 개선 보조로 활용한다[^9][^65].

### 6. 위궤양 치유 후 기능성 소화불량 동반군 (KCD-8: K30)

#### ① 코드
K30(기능성 소화불량)[교과서적 근거].

#### ② 병인병기
비기허·간위불화가 흔한 변증이며, 궤양 치유 후에도 비위 기능 저하가 지속되는 경우가 있다[교과서적 근거].

#### ③ 병태생리
위 배출 지연·내장 과민성·위저부 순응도 저하 등이 관여하며, 궤양의 기질적 치유와 기능적 회복은 별개일 수 있다[교과서적 근거].

#### ④ 치료
위궤양 완치 후에도 소화불량·삶의 질 저하가 지속되는 환자에게 한약(반하사심탕·향사평위산)·침·추나를 병행한 통합 한방 치료가 효과적일 수 있다(증례)[^117]. 사군자탕 기반 처방이 비궤양성 기능성 소화불량에서 메타분석 수준의 유효성을 확인했다(메타분석)[^97].

#### ⑤ 예후
기질적 병변이 없음을 확인한 후 변증에 따른 치료를 지속하면 증상·삶의 질 개선을 기대할 수 있다[^97][^117].

#### ⑥ 관리
정기적인 추적 관찰과 함께 식이·정서 관리, 재발 시 재내시경 검사를 권장한다[교과서적 근거].

## 제6편 예후·관리

### 1. 예후 인자

한약·침·혈위도포 등 한의 중재는 위궤양의 증상 완화·궤양 치유·재발 방지에 유용하며, 특히 H. pylori 관련 궤양에서 제균 요법과의 병용이 재발률을 유의하게 낮춘다[^65][^78]. 다만 다수 메타분석이 포함 연구들의 질이 낮고 진단·평가 기준이 비표준화되어 있음을 지적하며[^77][^78], 치료 반응과 경과는 변증 유형·H. pylori 감염 여부·NSAIDs 사용·치료 순응도에 따라 개인차가 크다[^118][^37].

### 2. 회복 경과

궤양 치유는 통상 4~8주의 표준 치료 기간을 거치며, 내시경으로 치유를 확인한다[교과서적 근거]. H. pylori 제균 성공 시 재발률이 현저히 낮아지나, 제균 실패·재감염 시 재발 위험이 높다[^54][^91].

### 3. 관리 원칙

- H. pylori 양성이면 표준 제균 요법과 함께 청열·건비·화위 한약 및 중성약을 병용하여 제균율을 높이고 재발을 관리한다[^41][^65][^78].
...
| 항목 | 지도 내용 | 이론적 근거 |
|---|---|---|
| 식이 | 규칙적 소량 다식, 자극적·과염·과유 음식 제한, 금주 | 「飮食自倍, 腸胃乃傷」[교과서적 근거] |
| 정서 | 스트레스 관리, 간기울결 예방 | 「怒傷肝, 肝乘脾」[교과서적 근거] |
| 흡연 | 금연(점막 혈류·치유 저해) | [^32] |
| 수면 | 규칙적 수면, 야간 위산분비 리듬 고려 | [교과서적 근거] |
| 약물 | NSAIDs·아스피린 장기 복용 시 위점막보호제 병용, 의료진과 상의 없는 임의 복용 금지 | [교과서적 근거] |
| 추적 | 궤양 치유 확인 내시경, H. pylori 재검사 | [^65][^91] |
...
| 위험 | 내용 | 참고 |
|---|---|---|
| H. pylori 항생제 내성 | 클래리스로마이신 내성 증가로 제균 실패율 상승, 베르베린 등 병용이 보조적으로 고려됨 | [^41][^90] |
| 대황 등 사하(瀉下) 본초 남용 | 만성 설사·전해질 불균형 우려, 변증(허증) 감별 필요 | [교과서적 근거] |
| 감초 장기 투여 | 부종·저칼륨혈증·고혈압 위험, 반하사심탕 등 장기 복용 시 주의 | [교과서적 근거] |
| NSAIDs·항혈소판제 병용 | 출혈 위험 증가, 한약 병용 시에도 정기적 혈액검사 권장 | [^9] |
| 임신 중 금기 본초 | 대황·행인 등 사하·활혈 본초는 임신 중 신중 투여 | [교과서적 근거] |
| 중증 출혈·천공 | 한의 단독 치료 금기, 즉시 응급 의뢰 | [^9][^118] |
...
| 영역 | 추적 지표 |
|---|---|
| 궤양 치유 | 내시경 Sakita-Miwa 분류(A/H/S기), 궤양 크기 |
| H. pylori | 요소호기검사, 대변항원검사 |
| 증상 | 위완통 VAS, 소화불량 증상 척도(GSRS 등) |
| 염증 지표 | CRP, 위점막 생검 조직검사 |
| 변증 객관화(연구 목적) | 위점막 EGFR·EGF·VEGF·PCNA, IL-12·IFN-γ, HSP70/72 |
| 합병증 | 혈색소(Hb), 대변 잠혈, 복부 영상 |

이 표는 임상 틀이지 동일 근거수준의 권고가 아니다.

### 7. 환자 설명용 요약

> 위궤양은 위벽이 헐어서 상처가 난 상태입니다. 대부분 헬리코박터 파일로리균 감염이나 진통소염제 복용이 원인이며, 정해진 기간(보통 4~8주) 동안 위산분비 억제제를 꾸준히 복용하고 필요하면 균을 없애는 항생제 치료를 함께 받아야 합니다. 한약이나 침 치료는 이러한 표준 치료를 대신하는 것이 아니라, 위장 기능을 돕고 소화불량·재발을 줄이는 데 보조적으로 도움을 줄 수 있습니다. 스트레스·과식·자극적 음식·흡연·음주는 위궤양의 회복을 늦추므로 함께 관리하는 것이 중요하며, 검은 변이나 심한 복통이 생기면 즉시 병원에 가야 합니다.
...
오경상합 이론 기반 침 치료는 증상 개선·궤양 치유에서 양방 약물과 유사한 효과를 보인다(임상시험)[^8]. 다만 H. pylori 음전율은 낮아 제균이 우선인 경우 약물 병행이 필요하다[^8]. 십이지장궤양에서는 3주 침 치료와 생활 습관 개선으로 76% 완전 회복률을 보였다(임상시험)[^32]. 침+중약 병용은 중약 단독보다 유효율·제균율이 높고 재발률이 낮다는 메타분석도 있다[^93].

**Q3. H. pylori 관련 위궤양에 중성약을 병용해도 되나요?**

H. pylori 관련 위궤양·위염에서 중성약+양약 병용이 양약 단독보다 효과적이고 안전하며, 목표 지표별로 특정 중성약이 유리하다는 네트워크 메타분석이 다수 있다[^77][^78][^87]. 다만 항생제 제균요법을 대체하는 것이 아니라 병용하는 것이 원칙이다.

**Q4. 변증(辨證)이 위궤양 치료에서 왜 중요한가요?**

한열변증 상태가 위점막 염증·산화 스트레스 지표와 밀접한 관련이 있으며(관찰연구)[^37], 변증(간위불화·비위허약·비위습열)에 따른 맞춤 혈위·처방 선택이 치료 성패의 핵심이다(임상시험)[^8]. 변증 없는 관행적 처방은 근거에 부합하지 않는다.

**Q5. 위궤양 완치 후에도 증상이 남는 환자에게 어떤 치료가 도움이 되나요?**

위궤양 완치 후 지속되는 소화불량에 한약(반하사심탕·향사평위산)·침·추나를 병행한 통합 한방 치료가 증상·삶의 질을 개선한다(증례)[^117]. 사군자탕 기반 처방도 비궤양성 기능성 소화불량에서 메타분석 수준의 유효성이 확인되었다[^97].

**Q6. 위궤양 출혈이나 천공이 의심되면 어떻게 해야 하나요?**

흑색변·토혈·심한 복통·복막염 징후는 응급 상황이므로 즉시 양방 응급실로 이송해야 한다[^9][^118]. 한의 단독 치료는 이러한 합병증에 대한 근거가 없으며, 안정화 이후 회복기 관리에 한해 보조적으로 활용할 수 있다.

**Q7. H. pylori 제균에 실패했는데 한의 치료가 도움이 될까요?**

중의약 기반 요법이 H. pylori 제균 실패 후 재치료(구제요법)에서 보조적 유효성을 보인 메타분석이 있다[^54]. 다제내성 H. pylori 감염 관리에서도 보완대체의학이 참고될 수 있다[^52]. 다만 항생제 감수성 검사에 기반한 표준 구제요법이 우선되어야 한다.

**Q8. NSAIDs를 오래 복용해야 하는데 위궤양을 예방할 방법이 있나요?**

NSAIDs 장기 복용 시에는 위점막보호제 병용이 표준이며, 건비화위 한약이 보조적으로 고려될 수 있으나 근거 수준이 제한적이므로 변증에 근거한 개별화된 접근이 필요하다[^103]. 정기적인 증상 모니터링과 필요시 내시경 검사를 권장한다.

---

**고전 인용 출처**: 『黃帝內經素問』(擧痛論, 至眞要大論), 『靈樞』(邪氣藏府病形), 『傷寒論』(半夏瀉心湯 관련 조문), 『金匱要略』(嘔吐噦下利病脈證治), 『脾胃論』(李東垣), 『景岳全書』(李梴)

**문헌 데이터 출처**: [한의학 논문 데이터베이스 (med.symbolicinfo.com)](https://med.symbolicinfo.com) — 2026-08-25 조회 기준

[^1]: Herbal Approaches to Ulcer Management: Mechanistic Pathways, Bioactive Constituents, and Clinical Outcomes. Pravin P. Pawar 외. _International Journal of Advanced Research in Science Communication and Technology_. 2025-12-03. [문헌 고찰] [DOI 10.48175/ijarsct-30130](https://doi.org/10.48175/ijarsct-30130) — 항궤양 본초의 기전 경로·생리활성 성분·임상 결과를 정리한 문헌 고찰.
[^2]: [Effects of chronoacupuncture na ja fa on gastric acid secretion, plasma gastrin and prostaglandin E1 in patients with peptic ulcer]. Cheng X 외. _Zhong xi yi jie he za zhi = Chinese journal of modern developments in traditional medicine_. 1991-02. [임상시험] [PMID 2060057](https://pubmed.ncbi.nlm.nih.gov/2060057/) — 자오유주(子午流注) 시간 침법이 위산분비·가스트린·PGE1을 조절하여 소화성궤양 치료에 활용될 수 있음을 시사.
[^3]: Efficacy and safety of Hou Gu Mi Xi for peptic ulcer diseases: Study protocol for a randomized controlled trial. Chen X 외. _Medicine_. 2019-07. [임상시험] [DOI 10.1097/MD.0000000000016561](https://doi.org/10.1097/MD.0000000000016561) [PMID 31335741](https://pubmed.ncbi.nlm.nih.gov/31335741/) — 후곡미희 계열 처방의 소화성궤양 대상 RCT 프로토콜.
[^4]: Alteration of the tongue manifestation reflects clinical outcomes of peptic ulcer disease. Wang HH 외. _Journal of alternative and complementary medicine (New York, N.Y.)_. 2012-11. [관찰연구] [DOI 10.1089/acm.2010.0706](https://doi.org/10.1089/acm.2010.0706) [PMID 23153037](https://pubmed.ncbi.nlm.nih.gov/23153037/) — 설진 소견 변화가 소화성궤양의 임상 경과와 상관관계를 보여 변증 객관화의 근거가 됨.
[^5]: Evidence construction of Chinese herbal formulae for the treatment of H. pylori positive peptic ulcer: A Bayesian network Meta-analysis. Yang J 외. _Phytomedicine : international journal of phytotherapy and phytopharmacology_. 2022-10. [메타분석] [DOI 10.1016/j.phymed.2022.154327](https://doi.org/10.1016/j.phymed.2022.154327) [PMID 35905565](https://pubmed.ncbi.nlm.nih.gov/35905565/) — H. pylori 양성 소화성궤양에 대한 다수 한약 처방의 상대적 유효성을 베이지안 네트워크 메타분석으로 비교.
[^6]: Prescription patterns of traditional Chinese medicine for peptic ulcer disease in Taiwan: A nationwide population-based study. Huang CY 외. _Journal of ethnopharmacology_. 2015-12-24. [관찰연구] [DOI 10.1016/j.jep.2015.11.002](https://doi.org/10.1016/j.jep.2015.11.002) [PMID 26549268](https://pubmed.ncbi.nlm.nih.gov/26549268/) — 대만 건강보험 빅데이터 분석으로 소화성궤양 환자의 한약 처방 실태를 확인.
[^57]: Effect of the folk remedy, Bainiku-ekisu, a concentrate of Prunus mume juice, on Helicobacter pylori infection in humans. Nakajima S 외. _Helicobacter_. 2006-12. [임상시험] [DOI 10.1111/j.1523-5378.2006.00463.x](https://doi.org/10.1111/j.1523-5378.2006.00463.x) [PMID 17083382](https://pubmed.ncbi.nlm.nih.gov/17083382/) — 오매 농축액이 H. pylori 감염 개선에 효과를 보고.
[^58]: Effect of Banxia Xiexin decoction on Helicobacter pylori-related peptic ulcers and its possible mechanism via the TGF-β/Smad signaling pathway. Chen S 외. _Journal of traditional Chinese medicine = Chung i tsa chih ying wen pan_. 2018-06. [실험연구] [PMID 32185975](https://pubmed.ncbi.nlm.nih.gov/32185975/) — 반하사심탕이 TGF-β/Smad 신호경로를 통해 H. pylori 관련 소화성궤양 개선 기전을 시사(인간 데이터 한정).
[^59]: Interleukin-12 and interferon-γ acting on damp-heat of spleen- stomach syndrome triggered by Helicobacter pylori. Chen W 외. _Journal of traditional Chinese medicine = Chung i tsa chih ying wen pan_. 2017-12. [관찰연구] [PMID 32188193](https://pubmed.ncbi.nlm.nih.gov/32188193/) — IL-12·IFN-γ가 비위습열증과 관련해 H. pylori 감염에 반응, 변증의 면역학적 근거를 뒷받침.
[^60]: MUC1 and MUC5AC Acting on Helicobacter pylori-Related Deficiency and Solid Syndrome of Spleen and Stomach. Hu L 외. _Evidence-based complementary and alternative medicine : eCAM_. 2018. [관찰연구] [DOI 10.1155/2018/9761919](https://doi.org/10.1155/2018/9761919) [PMID 29849739](https://pubmed.ncbi.nlm.nih.gov/29849739/) — MUC1·MUC5AC 발현이 비위 허실 변증과 관련되어 위점막 방어 기전의 변증적 차이를 시사.
[^61]: The Single-Nucleotide Polymorphism of miR-27a rs895819 and the Expression of miR-27a in Helicobacter pylori-Related Diseases and the Correlation with the Traditional Chinese Medicine Syndrome. Zhang L 외. _Evidence-based complementary and alternative medicine : eCAM_. 2022. [관찰연구] [DOI 10.1155/2022/3086205](https://doi.org/10.1155/2022/3086205) [PMID 35341140](https://pubmed.ncbi.nlm.nih.gov/35341140/) — miR-27a 유전자 다형성이 H. pylori 관련 질환의 중의 변증과 연관됨을 보고.
[^62]: Microecology-turbidity toxin theory: correlation between helicobacter pylori infection and manifestation of tongue and gastroscopy. Jin W 외. _Journal of traditional Chinese medicine = Chung i tsa chih ying wen pan_. 2022-06. [관찰연구] [DOI 10.19852/j.cnki.jtcm.20210612.001](https://doi.org/10.19852/j.cnki.jtcm.20210612.001) [PMID 35610017](https://pubmed.ncbi.nlm.nih.gov/35610017/) — 습탁독 이론에 기반한 미생태 상관성이 H. pylori 감염과 설진·위내시경 소견의 관련성을 시사.
...
[^84]: Research Progress on Traditional Chinese Medicine Interventions for Helicobacter Pylori. Gao Y 외. _Journal of Contemporary Medical Practice_. 2026-01-29. [체계적 고찰] [DOI 10.53469/jcmp.2026.08(01).20](https://doi.org/10.53469/jcmp.2026.08(01).20) — 중의약의 H. pylori 중재 연구 동향을 정리한 체계적 고찰.
...
[^86]: Korea red ginseng on Helicobacter pylori-induced halitosis: newer therapeutic strategy and a plausible mechanism. Lee JS 외. _Digestion_. 2009. [임상시험] [DOI 10.1159/000229997](https://doi.org/10.1159/000229997) [PMID 19776584](https://pubmed.ncbi.nlm.nih.gov/19776584/) — 고려홍삼이 H. pylori 유발 구취 개선에 기여하는 기전과 임상 효과를 제시.
[^87]: The comparative efficacy and safety of 9 traditional Chinese medicines combined with standard quadruple therapy for Helicobacter pylori-associated gastritis: a systematic review and network meta-analysis. Bao Z 외. _Annals of translational medicine_. 2022-12. [메타분석] [DOI 10.21037/atm-22-5421](https://doi.org/10.21037/atm-22-5421) [PMID 36660649](https://pubmed.ncbi.nlm.nih.gov/36660649/) — 9종 중성약과 표준 4제요법 병용의 상대적 유효성·안전성을 네트워크 메타분석으로 비교.
[^88]: Efficacy and safety of adjunctive Chinese herbal decoction in treating Helicobacter pylori–positive chronic atrophic gastritis: a real-world retrospective study. Wang J 외. _Frontiers in Medicine_. 2026-01-12. [관찰연구] [DOI 10.3389/fmed.2025.1701915](https://doi.org/10.3389/fmed.2025.1701915) — 중약 병용이 H. pylori 양성 만성 위축성 위염의 실제 임상 치료 성적을 개선.
[^89]: Association of miR-499 rs3746444, miR-149 rs2292832 polymorphisms and their expression levels with helicobacter pylori-related gastric diseases and Traditional Chinese Medicine syndromes. Qi L 외. _Journal of traditional Chinese medicine = Chung i tsa chih ying wen pan_. 2024-10. [관찰연구] [DOI 10.19852/j.cnki.jtcm.2024.05.009](https://doi.org/10.19852/j.cnki.jtcm.2024.05.009) [PMID 39380234](https://pubmed.ncbi.nlm.nih.gov/39380234/) — miR-499·miR-149 다형성과 발현이 H. pylori 관련 위질환의 중의 변증 유형과 연관.
[^90]: Real-World Evidence on Different Amoxicillin-Containing Regimens for Helicobacter pylori Treatment in Elderly Patients: Analysis of Efficacy, Safety, and Virulence Gene Association. Chen XL 외. _Antibiotics (Basel, Switzerland)_. 2026-03-30. [관찰연구] [DOI 10.3390/antibiotics15040355](https://doi.org/10.3390/antibiotics15040355) [PMID 42041318](https://pubmed.ncbi.nlm.nih.gov/42041318/) — 아목시실린 기반 요법의 실제 임상 근거를 고령 환자 대상으로 분석, 독성유전자와의 연관성을 확인.
...
[^99]: Rapid fire needling combined with Xiaoliu Jin Jianwei decoction improves gastrointestinal symptoms and gastric mucosal lesions in chronic atrophic gastritis. Li T 외. _American journal of translational research_. 2026. [임상시험] [DOI 10.62347/SBUR3437](https://doi.org/10.62347/SBUR3437) [PMID 42007096](https://pubmed.ncbi.nlm.nih.gov/42007096/) — 화침+소류금건위탕 병용이 만성 위축성 위염의 위장관 증상·위점막 병변을 개선.
