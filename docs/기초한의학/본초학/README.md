# 본초학 (本草學)

약재·단미재료 문서. 한의과대학 교과서적 표준 명칭(한글 + 한자)을 사용한다. 파일명 예: `인삼(人蔘).md`

**목차 구성 원칙** : 아래 목차는 한의과대학 공통 본초학 교재(영림사 본초학)의 효능별 분류 체계(해표약解表藥부터 외용약外用藥까지 20개 대분류)를 그대로 따라 구성했다. 각 분류 아래 기존 작성 문서를 매칭하고, 해당 분류의 대표 본초 중 미작성 항목은 "(작성 예정)"으로 병기해 교과서 목차 대비 누락을 확인할 수 있게 했다. 한 본초가 여러 효능을 겸하는 경우(예: 작약 — 보혈약/청열량혈약, 지황 — 청열량혈약/보혈약) 주된 분류에 배치하고 겸하는 분류에는 텍스트로 교차 표기한다.

## 1. 해표약(解表藥, Exterior-Releasing Herbs)

### 1-1. 발산풍한약(發散風寒藥, Wind-Cold Dispersing)

- 마황(麻黃, Ephedrae Herba)
- 계지(桂枝, Cinnamomi Ramulus)
- 자소엽(紫蘇葉, Perillae Folium)
- 형개(荊芥, Schizonepetae Herba)
- 강활(羌活, Notopterygii Rhizoma) — **작성됨**
- 백지(白芷, Angelicae Dahuricae Radix)
- 방풍(防風, Saposhnikoviae Radix)
- 고본(藁本, Ligustici Tenuissimi Rhizoma et Radix) — **작성됨**
- 신이(辛夷, Magnoliae Flos) — **작성됨**
- 세신(細辛, Asarum)
- 생강(生薑, Zingiberis Rhizoma Crudus) — **작성됨**
- 향유(香薷, Elsholtziae Herba) — **작성됨**
- 총백(蔥白, Allii Fistulosi Bulbus)
- 정류(荊芥穗, Schizonepetae Spica) — (작성 예정)
- 창이자(蒼耳子, Xanthii Fructus)

### 1-2. 발산풍열약(發散風熱藥, Wind-Heat Dispersing)

- 박하(薄荷, Menthae Herba) — **작성됨**
- 우방자(牛蒡子, Arctii Fructus) — **작성됨**
- 상엽(桑葉, Mori Folium) — **작성됨**
- 국화(菊花, Chrysanthemi Flos) — **작성됨**
- 갈근(葛根, Puerariae Radix)
- 시호(柴胡, Bupleuri Radix)
- 승마(升麻, Cimicifugae Rhizoma)
- 만형자(蔓荊子, Viticis Fructus) — **작성됨**
- 담두시(淡豆豉, Sojae Semen Praeparatum) — **작성됨**
- 선태(蟬蛻, Cicadae Periostracum) — **작성됨**
- 부평(浮萍, Spirodelae Herba)
- 목적(木賊, Equiseti Hiemalis Herba)

## 2. 청열약(淸熱藥, Heat-Clearing Herbs)

### 2-1. 청열사화약(淸熱瀉火藥, Fire-Purging)

- 석고(石膏, Gypsum Fibrosum)
- 지모(知母, Anemarrhenae Rhizoma)
- 노근(蘆根, Phragmitis Rhizoma)
- 천화분(天花粉, Trichosanthis Radix)
- 죽엽(竹葉, Bambusae Folium) — **작성됨** (기원식물 *Phyllostachys nigra* var. *henonis* 등. 담죽엽淡竹葉·*Lophatherum gracile*과 기원 식물이 다름을 문서 서두에서 구분)
- 치자(梔子, Gardeniae Fructus)
- 하고초(夏枯草, Prunellae Spica)
- 담죽엽(淡竹葉, Lophatheri Herba) — **작성됨** (정식 기원식물 *Lophatherum gracile*. 죽엽과의 기원 구분은 문서 서두 참조)
- 밀몽화(密蒙花, Buddlejae Flos)
- 청상자(靑箱子, Celosiae Semen) — **작성됨**
- 괴각(槐角, Sophorae Fructus) — **작성됨**

### 2-2. 청열조습약(淸熱燥濕藥, Heat-Clearing Damp-Drying)

- 황금(黃芩, Scutellariae Radix)
- 황백(黃柏, Phellodendri Cortex)
- 황련(黃連, Coptidis Rhizoma)
- 용담초(龍膽草, Gentianae Radix) — **작성됨**
- 고삼(苦蔘, Sophorae Flavescentis Radix) — **작성됨**
- 백선피(白鮮皮, Dictamni Cortex) — **작성됨**
- 대두황권(大豆黃卷, Sojae Semen Germinatum) — **작성됨**

### 2-3. 청열량혈약(淸熱涼血藥, Heat-Clearing Blood-Cooling)

- 서각/수우각(犀角/水牛角, Bubali Cornu) — **작성됨**
- 지황(地黃, Rehmanniae Radix) — 생지황·숙지황 하위 표제어 포함 **작성됨** (겸: 보혈약)
- 현삼(玄蔘, Scrophulariae Radix) — 명칭이 유사한 현호색(玄胡索)·연호색(延胡索)과 혼동 주의 **작성됨**
- 목단피(牡丹皮, Moutan Radicis Cortex)
- 적작약(赤芍藥) — 작약(芍藥, Paeoniae Radix) 문서에서 하위 표제어로 포함 **작성됨**
- 자초(紫草, Lithospermi Radix) — **작성됨**

### 2-4. 청열해독약(淸熱解毒藥, Heat-Toxin Clearing)

- 금은화(金銀花, Lonicerae Flos)
- 연교(連翹, Forsythiae Fructus)
- 포공영(蒲公英, Taraxaci Herba) — **작성됨**
- 자화지정(紫花地丁, Violae Herba) — **작성됨**
- 대청엽(大靑葉, Isatidis Folium)
- 판람근(板藍根, Isatidis Radix)
- 청대(靑黛, Indigo Pulverata Levis)
- 우황(牛黃, Bovis Calculus) — **작성됨**
- 마치현(馬齒莧, Portulacae Herba) — (작성 예정)
- 백두옹(白頭翁, Pulsatillae Radix) — (작성 예정)
- 진피(秦皮, Fraxini Cortex) — **작성됨**
- 패장(敗醬草, Patriniae Herba) — (작성 예정)
- 백화사설초(白花蛇舌草, Hedyotidis Herba) — **작성됨**
- 웅담(熊膽, Ursi Fel) — **작성됨**
- 백렴(白蘞, Ampelopsis Radix) — **작성됨**
- 누로(漏蘆, Rhapontici Radix) — **작성됨**
- 산자고(山慈姑, Cremastrae Pseudobulbus) — **작성됨**
- 녹두(綠豆, Phaseoli Radiati Semen) — (작성 예정)
- 위릉채(委陵菜, Potentillae Chinensis Herba) — **작성됨**
- 야국화(野菊花, Chrysanthemi Indici Flos) — (작성 예정)
- 인동등(忍冬藤, Lonicerae Caulis) — (작성 예정)
- 조휴(蚤休, Paridis Rhizoma) — (작성 예정)
- 반변련(半邊蓮, Lobeliae Chinensis Herba) — (작성 예정)
- 토복령(土茯苓, Smilacis Rhizoma) — **작성됨** (KHP 등재명은 Smilacis Rhizoma이며 기원종은 청미래덩굴·광엽발계 2종. 복령(茯苓)과 별개 약재)
- 어성초(魚腥草, Houttuyniae Herba) — **작성됨** (주사제 아나필락시스 근거와 경구 생약 근거를 분리해 서술)
- 사간(射干, Belamcandae Rhizoma) — (작성 예정)
- 산두근(山豆根, Sophorae Tonkinensis Radix) — (작성 예정)

### 2-5. 청허열약(淸虛熱藥, Deficiency-Heat Clearing)

- 청호(靑蒿, Artemisiae Annuae Herba) — **작성됨**
- 백미(白薇, Cynanchi Atrati Radix)
- 지골피(地骨皮, Lycii Cortex) — 구기자(枸杞子) 문서에서 하위 표제어로 포함 **작성됨**
- 은시호(銀柴胡, Stellariae Radix) — **작성됨**
- 호황련(胡黃連, Picrorhizae Rhizoma) — (작성 예정)

## 3. 사하약(瀉下藥, Purgative Herbs)

### 3-1. 공하약(攻下藥)

- 대황(大黃, Rhei Radix et Rhizoma) — **작성됨** (파일명: `대황(大黃, Rhei Rhizoma).md`)
- 망초(芒硝, Natrii Sulfas) — **작성됨**
- 노회(蘆薈, Aloe) — (작성 예정)

### 3-2. 윤하약(潤下藥)

- 화마인/마자인(火麻仁, Cannabis Semen) — (작성 예정)
- 욱리인(郁李仁, Pruni Semen) — (작성 예정)

### 3-3. 준하축수약(峻下逐水藥)

- 감수(甘遂, Kansui Radix) — (작성 예정)
- 대극(大戟, Euphorbiae Pekinensis Radix) — (작성 예정)
- 원화(芫花, Genkwa Flos) — (작성 예정)
- 견우자(牽牛子, Pharbitidis Semen) — (작성 예정)
- 상륙(商陸, Phytolaccae Radix) — (작성 예정)
- 파두(巴豆, Crotonis Fructus) — (작성 예정)
- 속수자(續隨子, Euphorbiae Lathyridis Semen) — (작성 예정)

## 4. 거풍습약(祛風濕藥, Wind-Damp Dispelling Herbs)

### 4-1. 거풍습지비통약(祛風濕止痺痛藥)

- 독활(獨活, Araliae Continentalis Radix)
- 위령선(威靈仙, Clematidis Radix) — (작성 예정)
- 방기(防己, Sinomeni Caulis et Rhizoma) — **작성됨** (KP 등재 기원은 청풍등 *Sinomenium acutum*. 중국약전의 분방기 *Stephania tetrandra* 및 목방기 *Cocculus*, 아리스토로크산 함유 광방기 *Aristolochia fangchi* 와의 구분은 문서 제1편 참조. 아리스토로크산 신병증·발암성 안전성 주의)
- 진교(秦艽, Gentianae Macrophyllae Radix) — (작성 예정)
- 해동피(海桐皮, Erythrinae Cortex) — (작성 예정)
- 초오(草烏, Aconiti Ciliare Tuber) — **작성됨** (KHP 기원은 이삭바꽃·놋젓가락나물·세잎돌쩌귀의 덩이뿌리. 포제품은 초오제. 아코니틴 안전성 주의 — 치료역이 좁고 국내 중독 사망 사례 보고)
- 잠사(蠶沙, Bombycis Faeces) — (작성 예정)
- 마전자(馬錢子, Strychni Semen) — (작성 예정, 스트리크닌 안전성 주의)

### 4-2. 서근활락약(舒筋活絡藥)

- 모과(木瓜, Chaenomelis Fructus) — (작성 예정)
- 낙석등(絡石藤, Trachelospermi Caulis) — (작성 예정)
- 상지(桑枝, Mori Ramulus) — (작성 예정)
- 금전백화사(金錢白花蛇, Bungarus Parvus) — (작성 예정)
- 기사(蘄蛇, Agkistrodon) — (작성 예정)
- 희렴초(豨薟草, Siegesbeckiae Herba) — (작성 예정)
- 오초사(烏梢蛇, Zaocys) — (작성 예정)
- 취오동(臭梧桐, Clerodendri Trichotomi Folium) — (작성 예정)

### 4-3. 거풍습강근골약(祛風濕强筋骨藥)

- 오가피(五加皮, Acanthopanacis Cortex) — (작성 예정)
- 호골(虎骨, Tigris Os) — (작성 예정)
- 곡기생(槲寄生, Visci Herba) — (작성 예정)
- 상기생(桑寄生, Taxilli Herba) — (작성 예정)

## 5. 방향화습약(芳香化濕藥, Aromatic Damp-Transforming Herbs)

- 창출(蒼朮, Atractylodis Rhizoma)
- 후박(厚朴, Magnoliae Cortex) — (겸: 이기약)
- 광곽향(廣藿香, Agastachis Herba) — (작성 예정)
- 사인(砂仁, Amomi Fructus) — (작성 예정)
- 백두구(白豆蔲, Amomi Fructus Rotundus) — (작성 예정)
- 초두구(草豆蔲, Alpiniae Katsumadai Semen) — (작성 예정)
- 초과(草果, Tsaoko Fructus) — (작성 예정)
- 패란(佩蘭, Eupatorii Herba) — (작성 예정)

## 6. 이수삼습약(利水滲濕藥, Water-Dispelling Damp-Percolating Herbs)

### 6-1. 이수퇴종약(利水退腫藥)

- 복령(茯苓, Poria Sclerotium) — 적복령·복신 하위 표제어 포함 **작성됨**
- 저령(猪苓, Polyporus) — (작성 예정)
- 택사(澤瀉, Alismatis Rhizoma)
- 의이인(薏苡仁, Coicis Semen) — **작성됨** (기원 *Coix lacryma-jobi* var. *ma-yuen*, 종피 제거. 의이인 유지 정맥주사제 강래특康萊特의 항종양 근거는 경구 단미 근거와 층위를 구분해 서술)
- 동과피(冬瓜皮, Benincasae Exocarpium) — (작성 예정)
- 적소두(赤小豆, Phaseoli Calcarati Semen) — (작성 예정)
- 옥미수(玉米鬚, Maydis Stigma) — (작성 예정)

### 6-2. 이뇨통림약(利尿通淋藥)

- 차전자(車前子, Plantaginis Semen) — **작성됨** (기원 *Plantago asiatica*·*P. depressa* 의 씨. 실리엄psyllium은 *P. ovata* 의 **씨껍질** 로 종·부위가 모두 달라 근거를 전용하지 않음. 지상부인 차전초車前草는 별개 품목)
- 목통(木通, Akebiae Caulis) — (작성 예정, 관목통 아리스톨로크산 안전성 문헌은 세신 문서에서 부분 다룸)
- 활석(滑石, Talcum) — (작성 예정)
- 통초(通草, Tetrapanacis Medulla) — (작성 예정)
- 해금사(海金沙, Lygodii Spora) — (작성 예정)
- 석위(石葦, Pyrrosiae Folium) — (작성 예정)
- 비해(萆薢, Dioscoreae Hypoglaucae Rhizoma) — (작성 예정)
- 지부자(地膚子, Kochiae Fructus) — (작성 예정)
- 편축(萹蓄, Polygoni Avicularis Herba) — (작성 예정)
- 구맥(瞿麥, Dianthi Herba) — (작성 예정)
- 동규자(冬葵子, Malvae Fructus) — (작성 예정)
- 등심초(燈心草, Junci Medulla) — (작성 예정)
- 삼백초(三白草, Saururi Herba) — (작성 예정)
- 인진호(茵蔯蒿, Artemisiae Capillaris Herba) — **작성됨** (효능상 이습퇴황약利濕退黃藥. 국내 규격집 등재명은 Artemisiae Capillaris Herba이고 기원은 사철쑥 *Artemisia capillaris* 단일 종. Artemisiae Scopariae Herba는 **중국약전 명칭** 으로 비쑥 *A. scoparia* 와 사철쑥을 함께 수재하므로 구분해 인용한다. 더위지기 *A. iwayomogi* 의 한인진韓茵蔯은 국내 규격집의 별개 품목)
- 금전초(金錢草, Lysimachiae Herba) — (작성 예정)

## 7. 온리약(溫裏藥, Interior-Warming Herbs)

- 부자(附子, Aconiti Lateralis Radix Praeparata)
- 천오두(川烏頭, Aconiti Radix) — (작성 예정)
- 건강(乾薑, Zingiberis Rhizoma)
- 육계(肉桂, Cinnamomi Cortex) — 계지(桂枝) 문서에서 하위 표제어로 포함 **작성됨**
- 오수유(吳茱萸, Evodiae Fructus) — (작성 예정)
- 촉초(蜀椒, Zanthoxyli Pericarpium) — (작성 예정)
- 필발(蓽茇, Piperis Longi Fructus) — (작성 예정)
- 필징가(蓽澄茄, Litseae Fructus) — (작성 예정)
- 정향(丁香, Caryophylli Flos) — (작성 예정)
- 고량강(高良薑, Alpiniae Officinarum Rhizoma) — (작성 예정)
- 소회향(小茴香, Foeniculi Fructus) — (작성 예정)
- 호초(胡椒, Piperis Nigri Fructus) — (작성 예정)

## 8. 이기약(理氣藥, Qi-Regulating Herbs)

- 진피(陳皮, Citri Unshius Pericarpium)
- 청피(靑皮, Citri Reticulatae Viride Pericarpium) — (작성 예정)
- 지실(枳實, Ponciri Fructus Immaturus)
- 지각(枳殼, Aurantii Fructus) — (작성 예정)
- 목향(木香, Aucklandiae Radix) — **작성됨** (CITES 부속서 I 등재종. 천목향·토목향과의 기원 혼용, 세스퀴테르펜 락톤 접촉피부염 주의)
- 향부자(香附子, Cyperi Rhizoma)
- 오약(烏藥, Linderae Radix) — (작성 예정)
- 침향(沈香, Aquilariae Lignum) — (작성 예정)
- 천련자(川楝子, Toosendan Fructus) — (작성 예정)
- 여지핵(荔枝核, Litchi Semen) — (작성 예정)
- 청목향(靑木香, Aristolochiae Radix) — (작성 예정, 아리스톨로크산 안전성 주의)
- 해백(薤白, Allii Macrostemonis Bulbus) — (작성 예정)
- 단향(檀香, Santali Albi Lignum) — (작성 예정)
- 시체(柿蔕, Kaki Calyx) — (작성 예정)
- 매괴화(玫瑰花, Rosae Rugosae Flos) — (작성 예정)
- 대복피(大腹皮, Arecae Pericarpium)
- 토목향(土木香, Inulae Radix) — (작성 예정)
- 감송향(甘松香, Nardostachyos Radix) — (작성 예정)

## 9. 소식약(消食藥, Digestant Herbs)

- 산사(山楂, Crataegi Fructus)
- 신곡(神麯, Massa Medicata Fermentata) — (작성 예정)
- 맥아(麥芽, Hordei Fructus Germinatus) — (작성 예정)
- 곡아(穀芽, Setariae Fructus Germinatus) — (작성 예정)
- 내복자(萊菔子, Raphani Semen) — (작성 예정)
- 계내금(鷄內金, Galli Gigerii Endothelium Corneum) — (작성 예정)

## 10. 구충약(驅蟲藥, Anthelmintic Herbs)

- 사군자(使君子, Quisqualis Fructus) — (작성 예정)
- 고련피(苦楝皮, Meliae Cortex) — (작성 예정)
- 빈랑자(檳榔子, Arecae Semen) — 대복피(大腹皮) 문서에서 동음이의 부위로 하위 표제어 포함(씨앗은 발암 위험 등 안전성 위주 서술, 구충 효능 독립 서술은 (작성 예정))
- 뇌환(雷丸, Omphalia) — (작성 예정)
- 학슬(鶴蝨, Carpesii Fructus) — (작성 예정)
- 비자(榧子, Torreyae Semen) — (작성 예정)
- 무이(蕪荑, Ulmi Macrocarpae Fructus) — (작성 예정)
- 관중(貫衆, Cyrtomii Rhizoma) — (작성 예정)

## 11. 지혈약(止血藥, Hemostatic Herbs)

### 11-1. 수렴지혈약(收斂止血藥)

- 선학초(仙鶴草, Agrimoniae Herba) — (작성 예정)
- 백급(白芨, Bletillae Rhizoma) — (작성 예정)
- 종려피(棕櫚皮, Trachycarpi Petiolus) — (작성 예정)
- 우절(藕節, Nelumbinis Nodus Rhizomatis) — (작성 예정)

### 11-2. 양혈지혈약(凉血止血藥)

- 대계(大薊, Cirsii Herba) — (작성 예정)
- 소계(小薊, Cirsii Herba) — (작성 예정)
- 지유(地楡, Sanguisorbae Radix) — (작성 예정)
- 괴화(槐花, Sophorae Flos) — (작성 예정)
- 측백엽(側柏葉, Platycladi Cacumen) — (작성 예정)
- 백모근(白茅根, Imperatae Rhizoma) — (작성 예정)
- 양제근(羊蹄根, Rumicis Radix) — (작성 예정)

### 11-3. 화어지혈약(化瘀止血藥)

- 삼칠(三七, Notoginseng Radix) — **작성됨**
- 포황(蒲黃, Typhae Pollen) — (작성 예정)
- 천초근(茜草根, Rubiae Radix) — (작성 예정)

### 11-4. 온경지혈약(溫經止血藥)

- 애엽(艾葉, Artemisiae Argyi Folium) — (작성 예정)

## 12. 활혈거어약(活血祛瘀藥, Blood-Invigorating Stasis-Dispelling Herbs)

- 천궁(川芎, Cnidii Rhizoma)
- 유향(乳香, Olibanum) — (작성 예정)
- 몰약(沒藥, Myrrha) — (작성 예정)
- 연호색(延胡索, Corydalis Tuber)
- 울금(鬱金, Curcumae Radix) — **작성됨**
- 강황(薑黃, Curcumae Longae Rhizoma) — **작성됨**
- 아출(莪朮, Curcumae Rhizoma) — **작성됨**
- 삼릉(三稜, Sparganii Rhizoma) — (작성 예정)
- 단삼(丹蔘, Salviae Miltiorrhizae Radix)
- 호장근(虎杖根, Polygoni Cuspidati Radix) — (작성 예정)
- 익모초(益母草, Leonuri Herba) — **작성됨**
- 도인(桃仁, Persicae Semen)
- 홍화(紅花, Carthami Flos)
- 오령지(五靈脂, Trogopterori Faeces) — (작성 예정)
- 우슬(牛膝, Achyranthis Radix)
- 천산갑(穿山甲, Manis Squama) — (작성 예정)
- 자충(蟅蟲, Eupolyphaga) — (작성 예정)
- 수질(水蛭, Hirudo) — (작성 예정)
- 택란(澤蘭, Lycopi Herba) — (작성 예정)
- 능소화(凌霄花, Campsis Flos) — (작성 예정)
- 자연동(自然銅, Pyritum) — (작성 예정)
- 왕불류행(王不留行, Vaccariae Semen) — (작성 예정)
- 소목(蘇木, Sappan Lignum) — (작성 예정)
- 건칠(乾漆, Lacca) — (작성 예정)
- 조각자(皂角刺, Gleditsiae Spina) — (작성 예정)
- 혈갈(血竭, Draconis Sanguis) — (작성 예정)
- 충울자(茺蔚子, Leonuri Fructus) — (작성 예정)
- 권백(卷柏, Selaginellae Herba) — (작성 예정)
- 계혈등(鷄血藤, Spatholobi Caulis) — **작성됨**

## 13. 화담지해평천약(化痰止咳平喘藥, Phlegm-Transforming Cough/Asthma-Relieving Herbs)

### 13-1. 온화한담약(溫化寒痰藥, Warm-Transforming Cold-Phlegm Herbs)

- 반하(半夏, Pinelliae Tuber)
- 천남성(天南星, Arisaematis Rhizoma) — (작성 예정)
- 백부자(白附子, Typhonii Rhizoma) — (작성 예정)
- 백개자(白芥子, Sinapis Albae Semen) — (작성 예정)
- 조협(皂莢, Gleditsiae Fructus) — (작성 예정)
- 선복화(旋覆花, Inulae Flos) — (작성 예정)
- 백전(白前, Cynanchi Stauntonii Rhizoma) — (작성 예정)

### 13-2. 청화열담약(淸化熱痰藥, Heat-Clearing Phlegm-Transforming Herbs)

- 전호(前胡, Peucedani Radix) — (작성 예정)
- 길경(桔梗, Platycodonis Radix)
- 과루(瓜蔞, Trichosanthis Fructus) — (작성 예정)
- 천패모(川貝母, Fritillariae Cirrhosae Bulbus) — (작성 예정)
- 절패모(浙貝母, Fritillariae Thunbergii Bulbus) — (작성 예정)
- 천축황(天竺黃, Bambusae Concretio Silicea) — (작성 예정)
- 죽여(竹茹, Bambusae Caulis in Taeniam) — (작성 예정)
- 죽력(竹瀝, Bambusae Succus) — (작성 예정)
- 청몽석(靑礞石, Chloriti Lapis) — (작성 예정)
- 해조(海藻, Sargassum) — (작성 예정)
- 곤포(昆布, Laminariae Thallus) — (작성 예정)
- 반대해(胖大海, Sterculiae Lychnophorae Semen) — (작성 예정)
- 비파엽(枇杷葉, Eriobotryae Folium) — (작성 예정)
- 동과자(冬瓜子, Benincasae Semen) — (작성 예정)

### 13-3. 지해평천약(止咳平喘藥, Cough/Asthma-Relieving Herbs)

- 행인(杏仁, Armeniacae Semen)
- 백부근(百部根, Stemonae Radix) — (작성 예정)
- 자완(紫菀, Asteris Radix) — (작성 예정)
- 관동화(款冬花, Farfarae Flos) — (작성 예정, 피롤리지딘 알칼로이드 안전성 문헌은 간정맥폐쇄성질환 문서에서 부분 다룸)
- 소자(蘇子, Perillae Fructus) — (작성 예정)
- 상백피(桑白皮, Mori Cortex) — (작성 예정)
- 정력자(葶藶子, Descurainiae Semen) — (작성 예정)
- 마두령(馬兜鈴, Aristolochiae Fructus) — (작성 예정, 아리스톨로크산 안전성 주의)
- 백과(白果, Ginkgo Semen) — **작성됨** (은행나무 *Ginkgo biloba* 의 **종자**. 은행엽 표준화 추출물과 기원 부위·근거가 전혀 다름을 문서 서두에서 구분. 긴코톡신 안전성 주의 — 소아 경련 위험)

## 14. 안신약(安神藥, Spirit-Calming Herbs)

- 주사(朱砂, Cinnabaris) — (작성 예정, 수은 안전성 주의)
- 자석(磁石, Magnetitum) — (작성 예정, 기초한의학 본초학 문서와 별개로 해부학 관련 자석磁石 문서가 있을 경우 교차 참조)
- 용골(龍骨, Fossilia Ossis Mastodi)
- 호박(琥珀, Succinum) — (작성 예정)
- 산조인(酸棗仁, Ziziphi Spinosae Semen)
- 백자인(柏子仁, Platycladi Semen) — (작성 예정)
- 원지(遠志, Polygalae Radix)
- 합환피(合歡皮, Albiziae Cortex) — (작성 예정)
- 영지(靈芝, Ganoderma)
- 야교등(夜交藤, Polygoni Multiflori Caulis) — (작성 예정)

## 15. 평간약(平肝藥, Liver-Calming Herbs)

### 15-1. 평간식풍약(平肝熄風藥, Liver-Calming Wind-Extinguishing Herbs)

- 영양각(羚羊角, Saigae Tataricae Cornu) — (작성 예정)
- 조구등(釣鉤藤, Uncariae Ramulus et Uncus)
- 천마(天麻, Gastrodiae Rhizoma)
- 백강잠(白殭蠶, Bombyx Batryticatus) — (작성 예정)
- 전갈(全蝎, Scorpio) — (작성 예정)
- 오공(蜈蚣, Scolopendra) — (작성 예정)
- 구인(蚯蚓, Pheretima) — (작성 예정)
- 결명자(決明子, Cassiae Semen) — (작성 예정)

### 15-2. 평간잠양약(平肝潛陽藥, Liver-Calming Yang-Subduing Herbs)

- 석결명(石決明, Haliotidis Concha) — (작성 예정)
- 모려(牡蠣, Ostreae Concha)
- 진주(珍珠, Margarita) — (작성 예정)
- 대자석(代赭石, Haematitum) — (작성 예정)
- 백질려(白蒺藜, Tribuli Fructus) — (작성 예정)

## 16. 개규약(開竅藥, Orifice-Opening Herbs)

- 사향(麝香, Moschus) — (작성 예정)
- 빙편(冰片, Borneolum) — (작성 예정)
- 석창포(石菖蒲, Acori Tatarinowii Rhizoma) — (작성 예정, 원지 문서에서 원지·석창포 상수 배오로 부분 언급됨)
- 소합향(蘇合香, Styrax) — (작성 예정)
- 안식향(安息香, Benzoinum) — (작성 예정)
- 섬소(蟾酥, Bufonis Venenum) — (작성 예정)
- 장뇌(樟腦, Camphora) — (작성 예정)

## 17. 보익약(補益藥, Tonifying Herbs)

### 17-1. 보기약(補氣藥)

- 인삼(人蔘, Ginseng Radix)
- 홍삼(紅蔘, Ginseng Radix Rubra) — 인삼의 뿌리를 증숙·건조한 약전 별도 수재 품목 **작성됨**
- 당삼(黨蔘, Codonopsis Radix) — (작성 예정)
- 황기(黃芪, Astragali Radix)
- 백출(白朮, Atractylodis Rhizoma Alba)
- 산약(山藥, Dioscoreae Rhizoma)
- 백편두(白扁豆, Lablab Semen Album) — (작성 예정)
- 감초(甘草, Glycyrrhizae Radix et Rhizoma)
- 대조(大棗, Zizyphi Fructus)
- 봉밀(蜂蜜, Mel) — (작성 예정)

### 17-2. 보양약(補陽藥)

- 녹용(鹿茸, Cervi Parvum Cornu) — **작성됨** (국내 등재명은 KHP `Cervi Parvum Cornu`. `Cervi Cornu Pantotrichum`은 중국약전 명칭이다. 기원은 매화록·마록·대록 3종이며 순록뿔은 기원종이 아니라 녹용절편 순도시험의 검출 금지 대상)
- 녹각(鹿角, Cervi Cornu) — (작성 예정)
- 녹각교(鹿角膠, Cervi Cornu Colla) — (작성 예정)
- 녹각상(鹿角霜, Cervi Cornu Degelatinatum) — (작성 예정)
- 파극천(巴戟天, Morindae Radix) — (작성 예정)
- 육종용(肉蓯蓉, Cistanches Herba) — (작성 예정)
- 선모(仙茅, Curculiginis Rhizoma) — (작성 예정)
- 음양곽(淫羊藿, Epimedii Herba)
- 호로파(葫蘆巴, Trigonellae Semen) — (작성 예정)
- 두충(杜仲, Eucommiae Cortex) — **작성됨**
- 속단(續斷, Dipsaci Radix) — (작성 예정)
- 보골지(補骨脂, Psoraleae Fructus) — (작성 예정)
- 구척(狗脊, Cibotii Rhizoma) — (작성 예정)
- 익지인(益智仁, Alpiniae Oxyphyllae Fructus) — (작성 예정)
- 골쇄보(骨碎補, Drynariae Rhizoma) — (작성 예정)
- 동충하초(冬蟲夏草, Cordyceps) — **작성됨** (KHP 등재명 `Cordyceps`. KHP 기원은 *Cordyceps sinensis*(현 *Ophiocordyceps sinensis*) 단일 종·박쥐나방과 유충 기생. 눈꽃동충하초 *C. militaris*와 균사체 발효물은 KHP 「동충하초」의 기원이 아니며 문서에서 근거를 분리해 서술)
- 합개(蛤蚧, Gecko) — (작성 예정)
- 호도인(胡桃仁, Juglandis Semen) — (작성 예정)
- 자하거(紫河車, Hominis Placenta) — (작성 예정)
- 토사자(菟絲子, Cuscutae Semen) — (작성 예정)
- 사원질려(沙苑蒺藜, Astragali Complanati Semen) — (작성 예정)
- 쇄양(鎖陽, Cynomorii Herba) — (작성 예정)
- 구자(韭子, Allii Tuberosi Semen) — (작성 예정)
- 양기석(陽起石, Actinolitum) — (작성 예정)
- 해구신(海狗腎, Callorhini Testis et Penis) — (작성 예정)
- 해마(海馬, Hippocampus) — (작성 예정)
- 사상자(蛇床子, Cnidii Fructus) — (작성 예정)

### 17-3. 보혈약(補血藥)

- 당귀(當歸, Angelicae Gigantis Radix)
- 숙지황(熟地黃) — 지황(地黃) 문서에서 하위 표제어로 포함 **작성됨**
- 백작약(白芍藥) — 작약(芍藥, Paeoniae Radix) 문서에서 하위 표제어로 포함 **작성됨** (겸: 청열량혈 — 적작약)
- 하수오(何首烏, Polygoni Multiflori Radix)
- 아교(阿膠, Asini Corii Colla) — (작성 예정)
- 용안육(龍眼肉, Longan Arillus) — (작성 예정)

### 17-4. 보음약(補陰藥)

- 사삼(沙蔘, Adenophorae/Glehniae Radix) — (작성 예정)
- 맥문동(麥門冬, Liriopis seu Ophiopogonis Tuber)
- 천문동(天門冬, Asparagi Radix) — (작성 예정)
- 석곡(石斛, Dendrobii Herba) — (작성 예정)
- 옥죽(玉竹, Polygonati Odorati Rhizoma) — (작성 예정)
- 황정(黃精, Polygonati Rhizoma) — (작성 예정)
- 백합(百合, Lilii Bulbus) — (작성 예정)
- 구기자(枸杞子, Lycii Fructus)
- 상심자(桑椹子, Mori Fructus) — (작성 예정)
- 한련초(旱蓮草, Ecliptae Herba) — (작성 예정)
- 여정자(女貞子, Ligustri Lucidi Fructus) — (작성 예정)
- 귀판(龜板, Testudinis Plastrum) — (작성 예정)
- 별갑(鱉甲, Trionycis Carapax) — (작성 예정)
- 흑지마(黑芝麻, Sesami Semen Nigrum) — (작성 예정)
- 저실자(楮實子, Broussonetiae Fructus) — (작성 예정)

## 18. 수삽약(收澁藥, Astringent Herbs)

### 18-1. 지한약(止汗藥)

- 부소맥(浮小麥, Tritici Fructus Levis) — (작성 예정)
- 마황근(麻黃根, Ephedrae Radix) — (작성 예정)
- 나도근(糯稻根, Oryzae Glutinosae Radix) — (작성 예정)

### 18-2. 지사약(止瀉藥)

- 가자(訶子, Chebulae Fructus) — (작성 예정)
- 육두구(肉豆蔲, Myristicae Semen) — (작성 예정)
- 적석지(赤石脂, Halloysitum Rubrum) — (작성 예정)
- 오매(烏梅, Mume Fructus) — (작성 예정)
- 앵속각(罌粟殼, Papaveris Pericarpium) — (작성 예정)
- 우여량(禹餘糧, Limonitum) — (작성 예정)
- 석류피(石榴皮, Granati Pericarpium) — (작성 예정)
- 춘피(椿皮, Ailanthi Cortex) — (작성 예정)
- 오배자(五倍子, Galla Chinensis) — (작성 예정)

### 18-3. 삽정축뇨지대약(澀精縮尿止帶藥)

- 오미자(五味子, Schisandrae Fructus)
- 연자육(蓮子肉, Nelumbinis Semen) — (작성 예정)
- 검실(芡實, Euryales Semen) — (작성 예정)
- 산수유(山茱萸, Corni Fructus) — (작성 예정)
- 금앵자(金櫻子, Rosae Laevigatae Fructus) — (작성 예정)
- 상표초(桑螵蛸, Mantidis Ootheca) — (작성 예정)
- 복분자(覆盆子, Rubi Fructus) — (작성 예정)
- 해표초(海螵蛸, Sepiae Endoconcha) — (작성 예정)
- 백반(白礬, Alumen) — (작성 예정)

## 19. 용토약(涌吐藥, Emetic Herbs)

- 과체(瓜蔕, Melo Pedicellus) — (작성 예정)
- 상산(常山, Dichroae Radix) — (작성 예정)
- 담반(膽礬, Chalcanthitum) — (작성 예정)
- 여로(藜蘆, Veratri Radix) — (작성 예정)

## 20. 외용약(外用藥, External Use Herbs)

- 유황(硫黃, Sulfur) — (작성 예정)
- 비석(砒石, Arsenicum) — (작성 예정, 비소 화합물 안전성 주의)
- 웅황(雄黃, Realgar) — (작성 예정, 비소 화합물 안전성 주의)
- 경분(輕粉, Calomelas) — (작성 예정, 수은 안전성 주의)
- 연단(鉛丹, Minium) — (작성 예정, 납 안전성 주의)
- 노감석(爐甘石, Calamina) — (작성 예정)
- 붕사(硼砂, Borax) — (작성 예정)
- 반모(斑蝥, Mylabris) — (작성 예정)
- 노봉방(露蜂房, Vespae Nidus) — (작성 예정)
- 대풍자(大風子, Chaulmoograe Semen) — (작성 예정)
- 목근피(木槿皮, Hibisci Cortex) — (작성 예정)

## 동음이의 분류

- `인삼(人蔘)`은 약재명으로, 식물종명 `Panax ginseng`를 가리킨다. 가공·제형에 따른 백삼·홍삼 구분은 별도 문서(홍삼(紅蔘, Ginseng Radix Rubra))와 하위 표제어로 다룬다. 홍삼은 대한민국약전에 인삼(Ginseng Radix)과 별개의 품목(Ginseng Radix Rubra)으로 수재되어 있으므로 파일명도 약전 수재 생약명을 따른다.
- `현삼(玄蔘, Scrophulariae Radix)`은 2-3 청열량혈약으로, 기원종은 *Scrophularia buergeriana*(및 근연종 *S. ningpoensis*)이다. 이름이 한 글자만 다른 `현호색(玄胡索)`(별칭 연호색 延胡索, 양귀비과 *Corydalis*속, 12 활혈거어약)과 표기·발음이 유사해 혼동의 소지가 있으므로, 기원 식물 학명을 반드시 병기한다. 두 약재의 관계는 동음이의가 아니라 명칭 유사이며, *Corydalis*를 "현삼"으로 적는 것은 오기이다.
- `작약(芍藥)`은 백작약(보혈약)과 적작약(청열량혈약)으로 효능이 갈리는 동일 속(*Paeonia*) 이물(異物) 관계이므로, 처방 배오 시 반드시 어느 쪽인지 구분해 서술한다.
- `연호색(延胡索)`은 본초학에서 현호색(玄胡索)으로도 표기되며, 12 활혈거어약에 속한다. 기원종 *Corydalis ternata*의 한국 표준 국명은 들현호색이다. 표기가 유사한 현삼(玄蔘, 2-3 청열량혈약)과 혼동하지 않도록 기원 학명을 병기한다.
