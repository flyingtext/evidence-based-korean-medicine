...
- `현삼(玄蔘)`은 두 가지 기원을 가진 동음이의어이다. 본초학의 현삼(玄蔘, *Scrophularia buergeriana*, 2-3 청열량혈약)과 연호색속 근연종인 *Corydalis ternata*의 한국명이 같아 혼동의 소지가 있으므로, 기원 식물 학명을 반드시 병기한다. 본 문서군에서는 *C. ternata*를 연호색(延胡索)의 하위 표제어로 다룬다.
- `작약(芍藥)`은 백작약(보혈약)과 적작약(청열량혈약)으로 효능이 갈리는 동일 속(*Paeonia*) 이물(異物) 관계이므로, 처방 배오 시 반드시 어느 쪽인지 구분해 서술한다.
- `연호색(延胡索)`은 본초학에서 현호색(玄胡索)으로도 표기되며, 12 활혈거어약에 속한다. 동음이의로 *Corydalis ternata*의 한국명 현삼(玄蔘)과 혼동하지 않도록 기원 학명을 병기한다.
