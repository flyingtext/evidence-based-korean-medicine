...
6. **근거 수준 우선 + 임상 참고 논문 필수 인용**: 논문 선별 시 최대한 높은 근거 수준(체계적 고찰/메타분석 > 임상시험 > 관찰연구 > 증례)의 논문을 우선 선별한다. 다만 근거 수준이 낮더라도 임상에 참고가 되는 논문(관찰연구·증례·파일럿 등)은 반드시 인용한다. 검색으로 확보한 관련 논문을 임의로 버리지 않고, 높은 근거 수준을 우선하되 임상적 가치가 있는 논문은 빠짐없이 근거 표·참고문헌·본문에 반영한다.
